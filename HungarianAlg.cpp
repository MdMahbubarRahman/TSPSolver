...
void HungarianAlg::diagonalSelection(std::set<int>& coveredRows, std::set<int>& coveredColumns, std::map<int, int>& boxPoints) {
		...
		if (it == coveredRows.end()) {
			for (int j = 0; j < reducedCostMatrixSize; j++) {
				...
				if (itt == coveredColumns.end() && reducedCostMatrix[i][j] == 0) {
					coveredRows.insert(j);
					boxPoints.insert(std::pair<int, int>(i, j));
					std::cout << "The new box point is : " << i << " , " << j << std::endl;
					rowID = i;
					colID = j;
					difVal = j - i;
					flag1 = false;
	...
	for (int i = rowID+1; i < reducedCostMatrixSize; i++) {
		auto it = coveredRows.find(i);
		if (it == coveredRows.end()) {
			for (int j = colID+1; j < reducedCostMatrixSize; j++) {
				auto ittt = coveredColumns.find(j);
				int val = j - i;
				if (ittt == coveredColumns.end() && val == difVal && reducedCostMatrix[i][j] == 0) {
					coveredRows.insert(j);
					boxPoints.insert(std::pair<int, int>(i, j));
					std::cout << "The new box point is : " << i << " , " << j << std::endl;
					flag2 = false;
...
//perform diagonal selection in case more than one zero in a row
void HungarianAlg::diagonalSelection1(std::set<int>& coveredRows, std::set<int>& coveredColumns, std::map<int, int>& boxPoints) {
	//1. first choose a zero in a row who has multiple zeros
	//2. //then if the sum of the number of covered rows and the number of covered columns is less than the size of the reduced matrix,
	//2. look for a second zero in a row which is positioned in diagonally opposite location
	//3. if the second zero containing row has two or more zeros, go to step 2
	...
	std::map<int, int> uncoveredRows;
	std::map<int, int> uncoveredColumns;
	int iter1 = 0;
	int iter2 = 0;
	for (int i = 0; i < reducedCostMatrixSize; i++) {
		auto it = coveredRows.find(i);
		if (it == coveredRows.end()) {
			uncoveredRows.insert(std::pair<int, int>(iter1, i));
			iter1 += 1;
		}
	}
	for (int i = 0; i < reducedCostMatrixSize; i++) {
		auto it = coveredColumns.find(i);
		if (it == coveredColumns.end()) {
			uncoveredColumns.insert(std::pair<int, int>(iter2, i));
			iter2 += 1;
		}
	}
	std::cout << "\nDiagonal selection in progress!" << std::endl;
		...
		if (it.first != rowID) {
			for (auto & itt: uncoveredColumns) {
				...
				if (itt.first != colID && val == difVal && reducedCostMatrix[it.second][itt.second] == 0 ) {
					coveredColumns.insert(it.second);
					boxPoints.insert(std::pair<int, int>(it.second, itt.second));
					std::cout << "The new box point is : " << it.second << " , " << itt.second << std::endl;
					flag2 = false;
					break;
...
//perform diagonal selection in case more than one zero in a row
void HungarianAlg::diagonalSelection2(std::set<int>& coveredRows, std::set<int>& coveredColumns, std::map<int, int>& boxPoints) {
	//1. first choose a zero in a row who has multiple zeros
	//2. //then if the sum of the number of covered rows and the number of covered columns is less than the size of the reduced matrix,
	//2. look for a second zero in a row which is positioned in diagonally opposite location
	//3. if the second zero containing row has two or more zeros, go to step 2
	...
	int difVal = 0;
	//perform first zero selection
	for (auto& it : uncoveredColumns) {
		for (auto& itt : uncoveredRows) {
			if (reducedCostMatrix[it.second][itt.second] == 0) {
				coveredRows.insert(itt.second);
				boxPoints.insert(std::pair<int, int>(itt.second, it.second));
				std::cout << "The new box point is : " << itt.second << " , " << it.second << std::endl;
				rowID = itt.first;
				colID = it.second;
				difVal = it.first - itt.first;
				flag1 = false;
				break;
			}
		}
	...
	for (auto& it : uncoveredColumns) {
		if (it.first != colID) {
			for (auto& itt : uncoveredRows) {
				int val = it.first - itt.first;
				if (itt.first != rowID && val == difVal && reducedCostMatrix[itt.second][it.second] == 0) {
					coveredRows.insert(itt.second);
					boxPoints.insert(std::pair<int, int>(itt.second, it.second));
					std::cout << "The new box point is : " << itt.second << " , " << it.second << std::endl;
					flag2 = false;
